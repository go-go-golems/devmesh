...
func main() {
	...
	fmt.Printf("P08 mixed loopback+wildcard rejected=%v (desired true)\n", e != nil)
	sock := filepath.Join(dir, "not-a-socket")
	must(os.WriteFile(sock, []byte("valuable file"), 0600))
	_, e = transport.Listen(sock)
	contents, readErr := os.ReadFile(sock)
	fmt.Printf("P09 regular socket path rejected=%v file_preserved=%v (desired true/true)\n", e != nil, readErr == nil && string(contents) == "valuable file")
	...
	_, allocErr := runtime.NewAllocator("127.0.0.1", 31000, 31100, store, logger).Allocate("durability", 0)
	_, diskErr := os.Stat(badPath)
	retry := store.SetPort("durability", 31000)
	fmt.Printf("P11 allocation persistence failure rejected=%v disk_missing=%v retry_rejected=%v (desired true/true/true)\n", allocErr != nil, diskErr != nil, retry != nil)
	...
	must(e)
	_, e = io.ReadFull(conn, buf)
	fmt.Printf("P12 established TCP after daemon Shutdown: read_error=%v payload=%s (desired closed promptly)\n", e, string(buf))
}
